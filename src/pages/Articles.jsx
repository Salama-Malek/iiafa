      ...
      <section className="py-16 md:py-24 pattern-overlay" style={{ background: 'var(--cream)' }}>
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-10">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${!selectedTag ? 'text-white' : 'text-gray-600 bg-white border border-gray-200 hover:border-[var(--primary-solid)]'}`}
                style={!selectedTag ? { background: 'var(--primary-solid)' } : {}}
              >
                الكل
              </button>
              {allTags.map(tag => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${selectedTag === tag ? 'text-white' : 'text-gray-600 bg-white border border-gray-200 hover:border-[var(--primary-solid)]'}`}
                  style={selectedTag === tag ? { background: 'var(--primary-solid)' } : {}}
                >
                  {tag}
                </button>
              ))}
          ...
          {!search && !selectedTag && featured.length > 0 && (
            <div className="mb-12">
              <h2 className="text-2xl font-bold mb-6" style={{ color: 'var(--primary-solid)' }}>مقالات مميزة</h2>
  ...
  return (
    ...
    >
      <Link
        ...
        className={`group block bg-white rounded-2xl overflow-hidden border border-transparent hover:border-[var(--primary-solid)]/30 shadow-sm hover:shadow-xl transition-all duration-500 ${featured ? 'md:flex' : ''}`}
      >
        {article.cover_image && (
        ...
        <div className={`p-5 md:p-6 ${featured ? 'md:w-1/2 flex flex-col justify-center' : ''}`}>
          ...
          <h3 className={`font-bold text-gray-900 group-hover:text-[var(--primary-solid)] transition-colors leading-relaxed ${featured ? 'text-xl md:text-2xl mb-3' : 'text-lg mb-2'}`}>
            {article.title}
          </h3>
          {article.excerpt && (
